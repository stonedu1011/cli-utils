...
if [ "$PROPERTIES" = "load" ]; then
  ...
  curl --request PUT -g -k -v  --data 'usermanagementservice' $CONSUL_ADDR/v1/kv/userviceconfiguration/defaultapplication/remoteservice.secretsservice.service 2>&1 1>/dev/null
  ...
  curl --request DELETE $CONSUL_ADDR/v1/kv/userviceconfiguration/defaultapplication/remoteservice.secretsservice.service 2>&1 1>/dev/null
